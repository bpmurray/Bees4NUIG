...
db.serialize(function() {
   ...
   db.run("CREATE TABLE IF NOT EXISTS beekeeper ( beekeeperid INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, name varchar(128) COLLATE NOCASE NOT NULL, email varchar(128) COLLATE NOCASE NOT NULL, password varchar(128) NOT NULL)");
   ...
   db.run("CREATE TABLE IF NOT EXISTS apiary ( apiaryid INTEGER NOT NULL, beekeeperid INTEGER REFERENCES beekeeper (beekeeperid) ON DELETE CASCADE, name varchar(128) COLLATE NOCASE NOT NULL, location varchar(256), colonies INTEGER, PRIMARY KEY (beekeeperid, apiaryid))");

   console.log("hive: Information about the beekeeper's hives in each apiary");
   db.run("CREATE TABLE IF NOT EXISTS hive ( queenid INTEGER NOT NULL, apiaryid INTEGER REFERENCES apiary (apiaryid) ON DELETE CASCADE, beekeeperid INTEGER REFERENCES beekeeper (beekeeperid) ON DELETE CASCADE, age INTEGER DEFAULT NULL, islocal INTEGER DEFAULT NULL, PRIMARY KEY (beekeeperid, apiaryid, queenid))");
